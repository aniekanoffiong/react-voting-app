...
class Results extends Component {
    ...
    totalVotes() {
        return this.store.getState().angular + this.store.getState().react + this.store.getState().vuejs; 
    }

    votesAngularInPercent() {
        if (this.store.getState().angular) {
            return (this.store.getState().angular / this.totalVotes()) * 100;
        } else {
            return 0;
        }
    }
    ...
    votesReactInPercent() {
        if (this.store.getState().react) {
            return (this.store.getState().react / this.totalVotes()) * 100;
        } else {
            return 0;
        }
    }
    ...
    votesVuejsInPercent() {
        if (this.store.getState().vuejs) {
            return (this.store.getState().vuejs / this.totalVotes()) * 100;
        } else {
            return 0;
        }
    }
